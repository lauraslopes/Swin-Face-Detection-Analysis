...
class FeatureAttentionModule(torch.nn.Module):
    def __init__(self, branch_num=3, in_chans=2112, feature_dim=512, conv_shared=False, conv_mode="split", kernel_size=3,
        ...
        if la_num_list is None:
            la_num_list = [2 for j in range(branch_num)]
        elif len(la_num_list) < branch_num:
            la_num_list = la_num_list + [2 for j in range(branch_num - len(la_num_list))]
